...
class Grapher(object):
    ...
    def get_image(self, width, height, format):
        ...
        w,h = [float(l)/72 for l in (width, height)]
    ...
    def figure(self):
        if not hasattr(self, '_figure'):
            ...
            ax = plt.Axes(self._figure, [0., 0., 1., 1.])
            ax.set_axis_off()
            self._figure.add_axes(ax)
            self.draw_axes(ax)
        return self._figure
...
class WaveGrapher(Grapher):
    ...
    def Y(self, restricted=True):
        ...
        sel = slice(bounds[0], bounds[1], step) if restricted else slice(0, None, step)
    ...
    def process_params(self, params):
        ...
        P['linewidth'] = float(params.get('linewidth', 0.5))
